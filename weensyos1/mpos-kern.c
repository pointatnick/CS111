...
interrupt(registers_t *reg)
{
	...
	case INT_SYS_EXIT:
		...
		if (current->p.wait)
		{
			proc_array[current->p_wait].p_registers.reg_eax = current->p_exit_status;
...
copy_stack(process_t *dest, process_t *src)
{
	...
	memcpy((void*)dest_stack_bottom, (void*)src_stack_bottom, (size_t)(src_stack_top - src_stack_bottom));
